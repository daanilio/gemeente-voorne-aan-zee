    ...
    <section class="grid grid-cols-1 md:grid-cols-1 gap-8">
        ...
        <div class="w-full md:w-1/2 mx-auto">
            <div class="bg-white shadow-lg p-4" id="formSection">
                <form method="post" action="{{ route('sessions') }}" enctype="multipart/form-data">
                    @csrf
                    @method('post')
                    <div class="mb-6">
                    ...
                    <div class="mb-6">
                        ...
                        <input
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            id="password" name="password" type="password" placeholder="Uw wachtwoord">
                        @error('password')
                        <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                        @enderror
                    </div>
                    <a href="{{ route('registreer') }}" class="text-blue-500">Nog geen account? Registreren</a>
